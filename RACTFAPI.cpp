...
void RACTFAPI::generateToken(const std::string& uname, const std::string& password, int otp) {
    ...
    if (otp == -1) {
        ...
        loginDetails[U("otp")] = web::json::value::number(otp);
    }
    ...
    auto req = apiClient.request(web::http::methods::POST, "/auth/login/", loginDetails.serialize(),
            ...
            if (response.status_code() != 200) {
                throw std::runtime_error("Endpoint returned " + std::to_string(response.status_code())
                        + " instead of 200.");
            }
